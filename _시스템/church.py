...
COPYRIGHT_NOTICE=("본 프로그램과 모든 참고자료의 저작권은 김용원 목사(세움교회)에게 있습니다. "
...
def manual(a):
    ...
    secs=[
     ...
     ("🆕 이번 업데이트로 새로워진 기능 (엑셀 명단·재정 엑셀·검수표·아이콘)",[
      "◆ 📥 교인 명단 한 번에 등록 — 이미 엑셀·한글로 갖고 계신 교인 명단을 통째로 불러옵니다. 한 분씩 입력하지 않으셔도 됩니다. (화면 ‘목양’ 섹션 / 번호 메뉴 21번) → 자세한 사용법은 아래 전용 항목을 보세요.",
      "◆ 💰 재정 엑셀 그대로 가져오기 — 재정부가 쓰시던 엑셀을 그대로 넣으시면 읽어서 결산서·기부금영수증까지 만들어 드립니다. (화면 ‘재정’ 섹션 / 번호 메뉴 22번) → 자세한 사용법은 아래 전용 항목을 보세요.",
      "◆ 📋 검수표 자동 생성 — 위 두 가지로 자료를 넣으실 때마다 ‘무엇이 들어가고 무엇이 빠졌는지’를 엑셀 검수표로 만들어 드립니다. 목사님이 교회 원장부와 직접 대조하실 수 있습니다.",
      "◆ 💾 백업 알림 — 백업하신 지 오래되면(또는 한 번도 안 하셨으면) 화면과 번호 메뉴에 알려 드립니다. 알림이 보이면 88번(자료 백업)을 한 번만 눌러 주세요.",
      "◆ 🖥 바탕화면 아이콘 자동 — 예전에는 「아이콘 만들기」 파일을 따로 실행하셔야 했지만, 이제는 프로그램을 켤 때 저절로 생깁니다. 교회 이름을 나중에 넣으셔도 아이콘 이름이 따라 바뀝니다.",
      "◆ 🇰🇷 영문 오류 없앰 — 문제가 생겨도 영어 대신 ‘무엇 때문인지·무엇을 하시면 되는지’를 한국어로 알려 드리고, 자료가 안전하다고 분명히 말씀드립니다. 기술적인 내용은 _오류기록 폴더에 파일로 남아, 필요할 때 담당자에게 보내시면 됩니다."]),
     ("📥 교인 명단 한 번에 등록 (엑셀·CSV) — 자세한 사용법",[
      "이미 엑셀이나 한글 표로 교인 명단을 갖고 계시면, 한 분씩 입력하실 필요 없이 통째로 등록됩니다. 새로 시작하시는 교회에서 가장 힘든 ‘명단 입력’을 한 번에 끝내는 기능입니다.",
      "① 명단 파일(.xlsx·.xls·.csv)을 프로그램 폴더에 두세요. (다른 곳에 있으면 전체 경로를 넣으셔도 됩니다)",
      "② 화면 ‘목양’ 섹션의 「교인 명단 한 번에 등록 (엑셀·CSV) 📥」 카드를 누릅니다. (번호 메뉴는 21번)",
      "③ ‘엑셀·CSV 파일’ 칸에 파일 이름을 넣고 그대로 실행합니다 — 이때는 미리보기라 자료가 바뀌지 않습니다.",
      "④ 화면에 ‘새로 등록 ○명 · 이미 있는 분 ○명’이 나옵니다. 숫자가 맞는지 확인하세요. 함께 만들어진 검수표(엑셀)를 열어 명단을 직접 대조하시면 가장 확실합니다.",
      "⑤ 맞으면 ‘실제로 등록’ 칸에 1 을 넣고 다시 실행합니다. → 등록됩니다.",
      "★ 열 제목이 달라도 알아서 맞춰 드립니다 — ‘성명·이름’, ‘휴대폰·연락처·전화’, ‘구역·소속셀·목장’처럼 교회마다 다른 이름을 알아봅니다. 이름 칸만 있으면 등록됩니다.",
      "★ 등록 직전에 자동으로 백업합니다. 백업에 실패하면 등록을 하지 않고 멈춥니다(자료를 지키기 위한 안전장치).",
      "★ 이미 등록된 분은 기본적으로 건너뜁니다. ‘이미 등록된 분’ 칸에 보완 을 넣으시면 ‘비어 있는 칸만’ 채웁니다 — 기존에 입력하신 정보는 절대 덮어쓰지 않습니다.",
      "★ 같은 이름이 두 분 계시거나 교적의 이름과 정확히 같지 않으면, 다른 분 정보가 섞이지 않도록 그대로 두고 그 명단을 알려 드립니다. 그때는 교우 카드에서 직접 확인해 주세요.",
      "※ ‘이름 칸을 찾지 못했습니다’라고 나오면 → 엑셀에서 이름이 들어 있는 열의 제목(첫 줄)을 ‘이름’으로 고치고 다시 넣어 주세요.",
      "※ 여러 장(시트)짜리 엑셀이면 ‘시트 이름’ 칸에 읽을 시트 이름을 넣어 주세요."]),
     ("💰 재정 엑셀 그대로 가져오기 — 자세한 사용법",[
      "재정부에서 엑셀로 정리해 오시던 장부를 그대로 넣으시면, 프로그램이 읽어서 연간 결산·항목별 통계·교인별 헌금대장·기부금영수증까지 만들어 드립니다. 재정부는 하시던 방식 그대로 하셔도 됩니다.",
      "◆ 세 가지 모양을 알아서 인식합니다:",
      "  ① 날짜 | 적요 | 수입 | 지출  (가장 흔한 거래 장부)",
      "  ② 항목 | 1월 | 2월 … 12월  (연간 결산표 — 이때는 ‘연도’ 칸에 2026 처럼 넣어 주세요)",
      "  ③ 날짜 | 십일조 | 감사헌금 | 선교헌금 …  (주간 헌금 집계표)",
      "① 화면 ‘재정’ 섹션의 「재정 엑셀 가져오기 💰」 카드를 누릅니다. (번호 메뉴는 22번)",
      "② 파일 이름을 넣고 그대로 실행 — 미리보기라 자료가 바뀌지 않습니다.",
      "③ 화면에 ‘수입 ○원 · 지출 ○원 · 차액 ○원’이 나옵니다. 이 합계가 교회 장부와 같은지 반드시 대조하세요(검수표가 함께 만들어집니다).",
      "④ 맞으면 ‘실제로 반영’ 칸에 1 을 넣고 다시 실행합니다. 반영 직전 자동으로 백업합니다.",
      "★ ‘합계·소계’ 줄은 자동으로 걸러냅니다. 금액은 1,200,000 · ₩ · 원 표기를 모두 알아봅니다.",
      "★ 같은 파일을 다시 넣어도 이미 들어간 내역은 건너뜁니다 — 매달 엑셀을 갱신해 다시 넣으셔도 안전합니다(중복으로 두 번 쌓이지 않습니다).",
      "★ 돈이 틀리게 들어갈 수 있는 상태에서는 아예 반영하지 않고 멈춥니다 — 예: 연도를 알 수 없는 월별 결산표, 마이너스 금액이 섞인 경우, 수입인지 지출인지 판단할 수 없는 ‘구분’ 값. 화면에 ⛔ 로 이유를 알려 드리니 그것만 고쳐 다시 넣으시면 됩니다.",
      "  · 마이너스 금액이 환불·정정으로 맞는 것이면 ‘마이너스허용’ 칸에 1 을 넣어 주세요.",
      "  · ‘구분’ 값을 못 알아보면 엑셀의 그 칸을 ‘수입’ 또는 ‘지출’로 고치시거나, 수입·지출을 각각 다른 열로 나눠 주세요.",
      "※ 반영하신 뒤에는 ‘주일 재정 결산서’·‘재정 출납부’·‘교인별 헌금대장’·‘연말정산 기부금영수증’을 그대로 뽑으실 수 있습니다."]),
     ("📋 검수표 — 내 장부와 대조하는 법 (자료를 믿고 쓰시려면)",[
      "위 두 가지(교인 명단·재정 엑셀)를 넣으실 때마다 엑셀 검수표가 자동으로 만들어집니다. 화면에 몇 줄 보여드리는 것만으로는 ‘내 장부와 정말 맞는지’ 확인할 수 없기 때문입니다.",
      "◆ 교인 일괄등록 검수표 (01 폴더에 저장): 검수 요약(읽은 줄 수·새로 등록할 분·이미 등록된 분) / 새로 등록할 명단 / 이미 등록된 분 / 옮기지 않은 줄.",
      "◆ 재정 가져오기 검수표 (06 폴더에 저장): 검수 요약(수입·지출·차액 합계) / 월별 대조 / 항목별 대조 / 옮기지 않은 줄.",
      "★ ‘옮기지 않은 줄’ 시트가 가장 중요합니다 — 어느 줄이 왜 안 들어갔는지 엑셀 줄 번호와 원본 내용까지 그대로 적어 드립니다.",
      "★ 보시는 순서: ①합계가 교회 장부와 같은가 → ②다르면 ‘옮기지 않은 줄’을 본다 → ③엑셀을 고쳐 다시 넣는다. 합계가 맞을 때까지 ‘실제로 반영’을 하지 않으시면 자료는 전혀 바뀌지 않습니다.",
      "※ 검수표는 미리보기·실제 반영 어느 쪽이든 항상 만들어집니다. 제직회·재정부에 보고하실 근거 자료로 그대로 쓰셔도 좋습니다."]),
     ("🆕 지난 업데이트로 새로워진 기능 (교적·목양·심방·화면)",[
      ...
      "◆ 바탕화면 아이콘 — 바탕화면에 ‘교회 행정’ 아이콘이 생깁니다. 이후엔 폴더를 열 필요 없이 그 아이콘만 더블클릭하세요. (이번 판부터는 프로그램을 켤 때 저절로 만들어집니다 — 아래 전용 항목 참고)"]),
     ...
     ("🖥️ 바탕화면 아이콘 — 저절로 생깁니다 (이번 판부터)",[
      "이제 목사님이 하실 일이 없습니다. 압축을 푸신 뒤 「★ 교회행정 시작 (여기를 더블클릭)」을 한 번 누르시면, 바탕화면에 시작 아이콘(교회 건물+십자가 모양)이 저절로 만들어집니다.",
      "① 그다음부터는 폴더를 열 필요 없이, 바탕화면의 그 아이콘만 더블클릭하면 프로그램이 바로 켜집니다.",
      "② 교회 이름을 아직 안 넣으신 동안에는 아이콘 이름이 ‘교회 종합행정’으로 보입니다. 나중에 교회 이름을 넣으시면 다음에 켤 때 ‘○○교회 행정’으로 저절로 바뀝니다(옛 아이콘은 정리됩니다).",
      "※ 아이콘이 안 보이면: 프로그램을 한 번 껐다 다시 켜 보세요. 켤 때마다 아이콘이 있는지 확인해서 없으면 다시 만들어 드립니다.",
      "※ 아이콘 그림이 예전 모양으로 보이면, 바탕화면 빈 곳을 마우스 오른쪽 클릭 → ‘새로 고침’을 한 번 누르시면 새 그림으로 바뀝니다.",
      "※ 예전 판에 있던 「★ 바탕화면 아이콘 만들기 (한 번만 더블클릭)」 파일도 그대로 두었습니다 — 눌러도 되지만, 이제는 누르지 않으셔도 됩니다."]),
     ...
     ("① 목양 — 성도 관리",[
      ...
      "★교인 명단 한 번에 등록(엑셀·CSV): 이미 갖고 계신 명단 파일을 통째로 불러옵니다 — 한 분씩 입력하지 않으셔도 됩니다(미리보기 → 검수표 확인 → 등록).",
     ...
     ("⑤ 재정",[
      ...
      "★재정 엑셀 가져오기: 재정부가 쓰시던 엑셀(거래 장부·연간 결산표·주간 헌금표)을 그대로 불러옵니다 — 미리보기·검수표로 장부와 대조한 뒤 반영하시면, 결산서·기부금영수증까지 그대로 나옵니다.",
